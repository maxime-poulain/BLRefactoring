...
/// <summary>
/// Puts a reinstated trainer's catalog back into the read-side search index, from the committed
/// fact.
/// </summary>
/// <remarks>
...
/// counterpart hid the entries instead of deleting them: an index that had forgotten the catalog
/// would have to be told it again, training by training, for a lifting that restores rather than
/// decides (ADR 0056).
/// </remarks>
public sealed class ShowCatalogWhenTrainerReinstatedIntegrationEventHandler(
    ITrainingSearchIndexer searchIndexer)
    : IIntegrationEventHandler<TrainerReinstatedIntegrationEvent>
{
    /// <inheritdoc />
    public string ConsumerName => "ShowReinstatedTrainerCatalog";
    ...
    public async Task HandleAsync(TrainerReinstatedIntegrationEvent integrationEvent, CancellationToken cancellationToken) =>
        await searchIndexer.ShowTrainerCatalogAsync(integrationEvent.TrainerId, cancellationToken);
}

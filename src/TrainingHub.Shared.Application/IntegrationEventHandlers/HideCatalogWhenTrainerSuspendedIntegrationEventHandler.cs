...
/// <summary>
/// Takes a suspended trainer's catalog out of the read-side search index, from the committed
/// fact.
/// </summary>
/// <remarks>
...
/// </para>
/// </remarks>
public sealed class HideCatalogWhenTrainerSuspendedIntegrationEventHandler(
    ITrainingSearchIndexer searchIndexer)
    : IIntegrationEventHandler<TrainerSuspendedIntegrationEvent>
{
    /// <inheritdoc />
    public string ConsumerName => "HideSuspendedTrainerCatalog";
    ...
    public async Task HandleAsync(TrainerSuspendedIntegrationEvent integrationEvent, CancellationToken cancellationToken) =>
        await searchIndexer.HideTrainerCatalogAsync(integrationEvent.TrainerId, cancellationToken);
}

    ...
    : CatalogControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        var page = await catalogApplicationService.SearchAsync(
            ...
            {
                Term = search?.Term,
                // Normalized once, in the mapping both stacks call: blanks dropped — an empty
                // query parameter asks for no filter rather than for a topic called nothing — and
                // duplicates collapsed, so ?topic=Design&topic=Design is one shelf (ADR 0080).
    ...
    public async Task<ActionResult<CatalogTopicsHttpResponse>> GetTopicsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var counted = await catalogApplicationService.GetFacetsAsync(facets?.Term, cancellationToken);
    ...
    public async Task<ActionResult<CatalogTrainerHttpResponse>> GetTrainerProfileAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var profile = await catalogApplicationService.FindOfferedTrainerAsync(
    ...
    public async Task<ActionResult> GetTrainerPortraitAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var portrait = await catalogApplicationService.FindTrainerPortraitAsync(
            trainerId, photoId, cancellationToken);

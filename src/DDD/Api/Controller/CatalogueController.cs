    ...
    : CatalogueControllerBase
{
    ...
    public async Task<ActionResult<CatalogueTrainingDetailHttpResponse>> GetOfferedTrainingAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var offered = await catalogueApplicationService.FindOfferedAsync(trainingId, cancellationToken);
    ...
    /// given it — they followed a catalogue entry, and a trainer's identifier is not something a
    /// catalogue hands out — and it is also what makes the response cacheable forever: a replacement
    ...
    public async Task<ActionResult> GetOfferedPortraitAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var portrait = await catalogueApplicationService.FindOfferedPortraitAsync(

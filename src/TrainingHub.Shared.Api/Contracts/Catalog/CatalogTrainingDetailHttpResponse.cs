...
/// <summary>
/// The body of <c>GET /Catalog/trainings/{id}</c>: one offered training, read in full (ADR 0062).
/// </summary>
/// <remarks>
/// The answer <see cref="CatalogTrainingHttpResponse"/> deliberately does not give. That row is
...
/// <para>
/// It carries the trainer's name and their identifier together, and each answers a different
/// question. The name is authorship — one name, attached to one training the trainer chose to
/// publish, because an offered course whose author is a GUID is a page nobody can use. The
/// identifier is where the name leads: the trainer's public page, which ADR 0070 opened. What the
/// row's old argument refused — <em>a listing of names, obtainable by paging</em> — stays refused;
/// what changed is that one person, already found through what they offer, now has an address.
/// </para>
/// <para>
...
/// photo. It waited on the precondition ADR 0021 recorded, that a photograph taken on a phone
/// carries the coordinates of where it was taken; ADR 0063 strips them and refuses to publish what
/// it cannot prove was stripped.
/// </para>
/// </remarks>
public sealed class CatalogTrainingDetailHttpResponse
{

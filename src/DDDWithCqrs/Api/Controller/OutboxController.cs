...
/// <summary>
/// The outbox's operator surface, on the CQRS stack: what delivery gave up on, and the one way back
/// in (ADR 0061).
/// </summary>
/// <remarks>
    ...
    : AdministrationControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        var page = await queryDispatcher.DispatchAsync(
            HttpToApplicationMappings.ToPoisonedMessagesQuery(pagination), cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        var result = await commandDispatcher.DispatchAsync(
            HttpToApplicationMappings.ToRequeueOutboxMessageCommand(messageId), cancellationToken);

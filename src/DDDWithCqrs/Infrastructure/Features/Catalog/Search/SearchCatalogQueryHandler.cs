...
namespace TrainingHub.DDDWithCqrs.Infrastructure.Features.Catalog.Search;

/// <summary>
/// Answers the catalog search by asking the search index the one question it answers.
/// </summary>
/// <remarks>
/// The only reader here that opens no <c>DbSet</c>, and the reason is what this repository means by
/// a bounded context. Its neighbors project columns out of the write model because that model is
...
/// </para>
/// </remarks>
public sealed class SearchCatalogQueryHandler(ITrainingSearchQuery trainingSearch)
    : IQueryHandler<SearchCatalogQuery, PagedResult<CatalogTrainingDto>>
{
    /// <summary>
    /// Runs the query.
    /// </summary>
    public async ValueTask<PagedResult<CatalogTrainingDto>> Handle(

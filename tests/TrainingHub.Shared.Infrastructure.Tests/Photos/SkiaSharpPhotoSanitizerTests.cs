...
/// </para>
/// </remarks>
public sealed class SkiaSharpPhotoSanitizerTests
{
    ...
    /// <summary>
    /// Sanitize, a photo carrying what a camera wrote, keeps none of it.
    /// </summary>
    /// <remarks>
    /// The fact ADR 0021 named and deferred, and the reason the portrait could not be published:
    /// <em>"a photo taken on a phone carries GPS coordinates, and a public catalog would publish
    ...
    /// </remarks>
    [Fact]
    public void Sanitize_APhotoCarryingWhatACameraWrote_KeepsNoneOfIt()
    {
        ...
        var stored = _sanitizer
            .Sanitize(uploaded, TrainerPhoto.JpegContentType)
            .ShouldBeSuccess();
    ...
    /// <summary>
    /// Sanitize, a portrait a camera recorded sideways, comes back upright.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    [Fact]
    public void Sanitize_APortraitRecordedSideways_ComesBackUpright()
    {
    ...
    /// <summary>
    /// Sanitize, an image larger than any page shows, is bounded and keeps its ratio.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    [Fact]
    public void Sanitize_AnImageLargerThanAnyPageShows_IsBoundedKeepingItsRatio()
    {
        ...
        width.Should().Be(SkiaSharpPhotoSanitizer.MaxDimension);
        height.Should().Be(SkiaSharpPhotoSanitizer.MaxDimension / 2);
    }

    /// <summary>
    /// Sanitize, an image already within the bound, keeps its dimensions.
    /// </summary>
    [Fact]
    public void Sanitize_AnImageAlreadyWithinTheBound_KeepsItsDimensions()
    {
    ...
    /// <summary>
    /// Sanitize, a PNG, is answered a PNG.
    /// </summary>
    /// <remarks>
    ...
    /// color the encoder chose.
    /// </remarks>
    [Fact]
    public void Sanitize_APng_IsAnsweredAPng()
    {
        var uploaded = Png(width: 40, height: 20);

        var stored = _sanitizer
            .Sanitize(uploaded, TrainerPhoto.PngContentType)
            .ShouldBeSuccess();
    ...
    /// <summary>
    /// Sanitize, bytes that carry a signature and do not decode, are refused by their own code.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    [Fact]
    public void Sanitize_BytesThatCarryASignatureAndDoNotDecode_AreRefusedByTheirOwnCode()
    {
        byte[] damaged = [0x89, (byte)'P', (byte)'N', (byte)'G', 0x0D, 0x0A, 0x1A, 0x0A, .. new byte[64]];

        _sanitizer
            .Sanitize(damaged, TrainerPhoto.PngContentType)

    ...
    private static string ResolveRoot()
    {
        ...
        if (string.IsNullOrWhiteSpace(stamped))
        {
            throw new InvalidOperationException(
                ...
                "BLRefactoring.Architecture.Tests.csproj as assembly metadata; without it every " +
                "rule that reads files would run over an empty set and pass.");
        }

        var root = Path.GetFullPath(stamped);

        if (!File.Exists(Path.Combine(root, "BLRefactoring.slnx")))
        {
            throw new InvalidOperationException(
                $"RepositoryRoot points at '{root}', which holds no BLRefactoring.slnx. A wrong " +

...
public sealed class ProjectGraphRules
{
    ...
    public void TheDomain_ReferencesTheKernelAndNothingElse() =>
        ProjectGraph.Project("BLRefactoring.Shared.Domain").ProjectReferences
            .Selected("reference on the domain project")
            .Where(reference => reference != "BLRefactoring.Shared")
    ...
    public void TheKernel_ReferencesNoProject() =>
        ProjectGraph.Project("BLRefactoring.Shared").ProjectReferences
    ...
    public void NeitherStack_ReferencesTheOther() =>
        ProjectGraph.Projects
            ...
            .SelectMany(project => project.ProjectReferences
                .Where(reference =>
                    (project.BelongsTo("DDD") && reference.StartsWith("BLRefactoring.DDDWithCqrs.", StringComparison.Ordinal))
                    || (project.BelongsTo("DDDWithCqrs") && reference.StartsWith("BLRefactoring.DDD.", StringComparison.Ordinal)))
    ...
    public void OnlyTheSharedApiLayer_CarriesTheWebFrameworkReference() =>
        ProjectGraph.Projects
            .Where(project => project.Name != "BLRefactoring.Shared.Api")
            ...
            .Where(project => !project.Name.EndsWith(".Api", StringComparison.Ordinal)
                              && !project.Name.StartsWith("BLRefactoring.Blazor", StringComparison.Ordinal)
                              && !project.Name.StartsWith("BLRefactoring.Architecture", StringComparison.Ordinal))
    ...
    {
        var client = ProjectGraph.Project("BLRefactoring.GeneratedClients");
...
/// labels are written short — <c>Shared.Domain</c>, not <c>BLRefactoring.Shared.Domain</c> — so the
...
internal static class MermaidGraph
{
    ...
    private static string Qualify(string label) =>
        label.StartsWith("BLRefactoring.", StringComparison.Ordinal) ? label : $"BLRefactoring.{label}";
}

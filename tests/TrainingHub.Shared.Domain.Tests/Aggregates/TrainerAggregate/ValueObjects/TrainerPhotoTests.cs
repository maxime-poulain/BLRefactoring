...
public sealed class TrainerPhotoTests
{
    ...
    /// SQLite, in <c>CatalogueDetailQueryTests</c>.
    /// </para>
    /// </remarks>
    [Fact]
    public void Create_AnAcceptedPhoto_RecordsWhenItWasStripped()
    {
        var photo = TrainerPhoto
            .Create(Png(), TrainerPhoto.PngContentType, SanitisedAt)
            .ShouldBeSuccess();

        photo.SanitisedOnUtc.Should().Be(SanitisedAt);
    ...
    {
        // Act
        var result = TrainerPhoto.Create(Png(), TrainerPhoto.PngContentType, SanitisedAt);
    ...
    {
        // Act
        var result = TrainerPhoto.Create(Jpeg(), TrainerPhoto.JpegContentType, SanitisedAt);
    ...
    {
        // Act
        var result = TrainerPhoto.Create(Webp(), TrainerPhoto.WebpContentType, SanitisedAt);
    ...
    public void Create_ValidPng_DescribesTheBytes()
    {
        ...
        // Act
        var photo = TrainerPhoto.Create(content, TrainerPhoto.PngContentType, SanitisedAt).ShouldBeSuccess();
    ...
    public void Create_CalledTwice_MintsAFreshIdentity()
    {
        ...
        // Act
        var first = TrainerPhoto.Create(content, TrainerPhoto.PngContentType, SanitisedAt).ShouldBeSuccess();
        var second = TrainerPhoto.Create(content, TrainerPhoto.PngContentType, SanitisedAt).ShouldBeSuccess();
    ...
    {
        // Act
        var result = TrainerPhoto.Create([], TrainerPhoto.PngContentType, SanitisedAt);
    ...
    public void Create_OneBytePastTheLimit_ReturnsFailure()
    {
        ...
        var result = TrainerPhoto.Create(
            Png(TrainerPhoto.MaxSizeInBytes + 1), TrainerPhoto.PngContentType, SanitisedAt);
    ...
    public void Create_ExactlyAtTheLimit_ReturnsSuccess()
    {
        // Act
        var result = TrainerPhoto.Create(
            Png(TrainerPhoto.MaxSizeInBytes), TrainerPhoto.PngContentType, SanitisedAt);
    ...
    {
        // Act
        var result = TrainerPhoto.Create(Jpeg(), TrainerPhoto.PngContentType, SanitisedAt);
    ...
    {
        // Act
        var result = TrainerPhoto.Create("GIF89a......."u8, TrainerPhoto.PngContentType, SanitisedAt);
    ...
    {
        // Act
        var result = TrainerPhoto.Create(Png(), null, SanitisedAt);
    ...
    {
        // Act
        var result = TrainerPhoto.Create(Jpeg(), "image/jpeg; charset=binary", SanitisedAt);
    ...
    {
        // Act
        var result = TrainerPhoto.Create(Png(), "IMAGE/PNG", SanitisedAt);
    ...
    {
        // Act
        var photo = TrainerPhoto.Create(Png(), "IMAGE/PNG", SanitisedAt).ShouldBeSuccess();
    ...
    {
        // Act
        var result = TrainerPhoto.Create("RIFF....WAVEfmt "u8, TrainerPhoto.WebpContentType, SanitisedAt);
    ...
    {
        // Act
        var result = TrainerPhoto.Create("RIFF..."u8, TrainerPhoto.WebpContentType, SanitisedAt);
    ...
    {
        // Act
        var result = TrainerPhoto.Create([0x89, 0x50, 0x4E], TrainerPhoto.PngContentType, SanitisedAt);
    ...
    /// </summary>
    /// <remarks>
    /// One of the two rules that exist only because this method does. Sanitisation bounds the
    ...
    /// sanitiser re-encodes into the format it is told, so a JPEG uploaded as <c>image/png</c>
    ...
    /// </summary>
    /// <remarks>
    /// What it answers is what the sanitiser is told to write back, so the format survives the
    /// round trip rather than being normalised — a trainer who uploads a PNG with a transparent

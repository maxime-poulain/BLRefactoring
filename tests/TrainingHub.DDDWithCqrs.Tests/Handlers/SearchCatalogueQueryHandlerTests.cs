...
using TrainingHub.DDDWithCqrs.Infrastructure.Features.Catalogue.Search;
...
/// <summary>
/// Behaviour covered for <c>SearchCatalogueQueryHandler</c>.
/// </summary>
/// <remarks>
...
/// </remarks>
public sealed class SearchCatalogueQueryHandlerTests
{
    ...
    public async Task Handle_ATermAndAPage_AsksTheIndexForExactlyThose()
    {
        ...
        _trainingSearch
            .Setup(search => search.SearchAsync("event storming", paging, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new PagedResult<CatalogueTrainingDto>([], 3, 5, 0));

        var sut = new SearchCatalogueQueryHandler(_trainingSearch.Object);

        await sut.Handle(
            new SearchCatalogueQuery { Term = "event storming", Paging = paging },
            CancellationToken.None);

        _trainingSearch.Verify(
            search => search.SearchAsync("event storming", paging, It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    public async Task Handle_ThePageTheIndexAnswered_HandsItBackUntouched()
    {
        ...
        _trainingSearch
            .Setup(search => search.SearchAsync(
                It.IsAny<string?>(), It.IsAny<PageRequest>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(answered);

        var sut = new SearchCatalogueQueryHandler(_trainingSearch.Object);

        var page = await sut.Handle(new SearchCatalogueQuery(), CancellationToken.None);

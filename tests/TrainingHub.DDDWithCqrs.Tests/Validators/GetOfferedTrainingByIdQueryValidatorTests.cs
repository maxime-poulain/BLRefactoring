...
/// </remarks>
public sealed class GetOfferedTrainingByIdQueryValidatorTests
{
    /// <summary>
    ...
    public void Validate_AnIdentifierThatNamesSomething_AcceptsIt() =>
        new GetOfferedTrainingByIdQueryValidator()
            .Validate(new GetOfferedTrainingByIdQuery(Guid.CreateVersion7()))
    ...
    {
        var result = new GetOfferedTrainingByIdQueryValidator()
            .Validate(new GetOfferedTrainingByIdQuery(Guid.Empty));

        result.IsValid.Should().BeFalse();
        result.Errors.Should().ContainSingle()
            .Which.PropertyName.Should().Be(nameof(GetOfferedTrainingByIdQuery.TrainingId));
    }
}

...
/// </summary>
/// <remarks>
/// Two claims, and neither is about the stripping itself — that is what the sanitiser's own tests
...
/// </remarks>
public sealed class PhotoSanitisationRules
{
    ...
    private const string CatalogueReader = "src/TrainingHub.Shared.Infrastructure/Search/CatalogueDetailQuery.cs";

    /// <summary>The stamp a publishable portrait carries, named as the reader must name it.</summary>
    private const string SanitisationStamp = "SanitisedOnUtc";
    ...
    /// </summary>
    /// <remarks>
    /// The precondition ADR 0062 named, made executable. What the public catalogue serves is bytes
    ...
    /// <para>
    /// Written against the reader's source rather than against its behaviour, and that is a
    ...
    {
        var path = Path.Combine(SourceTree.RepositoryRoot, CatalogueReader);

        File.Exists(path).Should().BeTrue(
            $"'{CatalogueReader}' is the adapter this rule is about, and a rule that cannot find " +
            "its subject passes by selecting nothing");

        var occurrences = SourceTree
            .ReadLines(path)
            .Count(line => line.Contains(SanitisationStamp, StringComparison.Ordinal)
                && !line.TrimStart().StartsWith("//", StringComparison.Ordinal));

        occurrences.Should().BeGreaterThan(0,
            $"'{CatalogueReader}' publishes a trainer's portrait to anybody and never names " +
            $"'{SanitisationStamp}'. A portrait stored before ADR 0063 carries whatever the camera " +
            "wrote, GPS included, and nothing but this predicate keeps it off the public catalogue");
    }
}

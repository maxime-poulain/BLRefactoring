...
/// <summary>
/// The catalog's reading of one training over HTTP, on both hosts (ADR 0062).
/// </summary>
/// <remarks>
/// The sibling of <see cref="CatalogSearchTest{TFactory}"/>, and it proves the half that suite
...
public abstract class CatalogDetailTest<TFactory>(TFactory factory) : IntegrationTest<TFactory>(factory)
    where TFactory : IResettableDatabase, IServiceScopeSource, IHttpClientSource
{
    ...
    public async Task AnOfferedTraining_IsReadInFullByACallerWithNoToken()
    {
        ...
        var read = await anonymous.GetAsync($"/Catalog/trainings/{trainingId}");

        read.StatusCode.Should().Be(HttpStatusCode.OK);

        var offered = await read.Content.ReadFromJsonAsync<CatalogTrainingDetailHttpResponse>();
    ...
    public async Task AWithheldTraining_AnswersTheSameNothingAsOneThatNeverExisted()
    {
        ...
        var neverExisted = await anonymous.GetAsync($"/Catalog/trainings/{Guid.CreateVersion7()}");
    ...
    public async Task TheEmptyIdentifier_IsRefusedRatherThanAnsweredWithANothing()
    {
        ...
        var refused = await anonymous.GetAsync($"/Catalog/trainings/{Guid.Empty}");
    ...
    /// something strips them, the domain records that it did, and a visitor who followed a catalog
    ...
    public async Task AnOfferedTrainingsPortrait_IsServedToACallerWithNoToken()
    {
        ...
        var portrait = await anonymous.GetAsync(
            $"/Catalog/trainings/{trainingId}/photo/{detail.TrainerPhotoId}");
    ...
    public async Task APhotoTheOwnerDoesNotHave_AnswersNothing()
    {
        ...
        var portrait = await anonymous.GetAsync(
            $"/Catalog/trainings/{trainingId}/photo/{Guid.CreateVersion7()}");
    ...
    public async Task AWithheldTraining_MakesItsOwnersPortraitUnreachable()
    {
        ...
        var portrait = await anonymous.GetAsync(
            $"/Catalog/trainings/{trainingId}/photo/{me.PhotoId}");
        ...
        HttpStatusCode expected)
    {
        ...
        while (DateTime.UtcNow < deadline)
        {
            last = (await anonymous.GetAsync($"/Catalog/trainings/{trainingId}")).StatusCode;

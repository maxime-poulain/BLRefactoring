    ...
    where TFactory : IResettableDatabase, IHttpClientSource, IServiceScopeSource
{
    ...
    public async Task ASuspendedTrainer_ComesBackExactlyAsTheyLeft()
    {
        ...
        // Suspended, the trainer's catalogue cannot grow again: the aggregate refuses to publish
        // for an owner under sanction, which is the standing rule ADR 0050 wrote.
        var refused = await trainer.PostAsync($"/Training/{withdrawn}/publish", content: null);
        refused.StatusCode.Should().Be(HttpStatusCode.BadRequest);
        await ShouldCarryAsync(refused, TrainingErrorCodes.TrainerSuspended);

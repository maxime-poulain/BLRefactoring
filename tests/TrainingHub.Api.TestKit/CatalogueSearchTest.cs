...
/// <summary>
/// The public catalogue over HTTP, on both hosts: what a visitor with no token can find, and what
...
public abstract class CatalogueSearchTest<TFactory>(TFactory factory) : IntegrationTest<TFactory>(factory)
    where TFactory : IResettableDatabase, IServiceScopeSource, IHttpClientSource
{
    ...
    public async Task APublishedTraining_BecomesFindableByAWordOfItsTitle_ToACallerWithNoToken()
    {
        ...
        (await WaitForCatalogueAsync("hexagonal", holds: trainingId)).Should().Contain(trainingId);
    }

    /// <summary>
    /// A withdrawn training, leaves the catalogue.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    [Fact]
    public async Task AWithdrawnTraining_LeavesTheCatalogue()
    {
        var trainer = await AuthHelper.RegisterAndGetAuthenticatedClientAsync(Factory);
        ...
        (await WaitForCatalogueAsync("withdrawn", holds: trainingId)).Should().Contain(trainingId);

        (await trainer.PostAsync($"/Training/{trainingId}/unpublish", content: null))
            .StatusCode.Should().Be(HttpStatusCode.NoContent);

        (await WaitForCatalogueAsync("withdrawn", holdsNot: trainingId)).Should().NotContain(trainingId);
    }

    /// <summary>
    /// A suspended trainer's catalogue, leaves public view, and comes back when the sanction is
    /// lifted.
    /// </summary>
    /// <remarks>
    ...
    /// deleted the entries would need the catalogue read back to rebuild them, which is precisely
    /// the design that record rejected.
    /// </remarks>
    [Fact]
    public async Task ASuspendedTrainersCatalogue_LeavesPublicView_AndComesBackWhenTheSanctionIsLifted()
    {
        var trainer = await AuthHelper.RegisterAndGetAuthenticatedClientAsync(Factory);
        var trainerId = (await trainer.GetFromJsonAsync<TrainerHttpResponse>("/Trainer/me"))!.Id;

        var created = await trainer.PostAsJsonAsync(
            "/Training", TrainingRequests.Valid("Sanctioned Catalogue Entry"));
        created.StatusCode.Should().Be(HttpStatusCode.Created);
        var trainingId = await created.Content.ReadFromJsonAsync<Guid>();

        (await WaitForCatalogueAsync("sanctioned", holds: trainingId)).Should().Contain(trainingId);
        ...
        (await WaitForCatalogueAsync("sanctioned", holdsNot: trainingId)).Should().NotContain(trainingId);
    ...
    public async Task ATermLongerThanATitle_IsRefusedRatherThanAnsweredWithAnEmptyPage()
    {
        ...
        var refused = await anonymous.GetAsync(
            $"/Catalogue/trainings?term={new string('a', 101)}");

        refused.StatusCode.Should().Be(HttpStatusCode.BadRequest);
    }

    /// <summary>
    /// Polls the public catalogue until it holds — or stops holding — the training, and answers the
    ...
    /// </remarks>
    private async Task<IReadOnlyList<Guid>> WaitForCatalogueAsync(
        string term,
        Guid? holds = null,
        Guid? holdsNot = null)
    {
        ...
        {
            var page = await anonymous.GetAsync($"/Catalogue/trainings?term={term}&page=1&pageSize=50");

...
| `GET` | `/Catalog/trainings` | **Anonymous.** One page of the offered catalog, by title and shelf. `?term=` matched against the words of a title, each by prefix, through the search index rather than the trainings table; `?topic=` narrowing to one shelf, refused when the domain does not spell it (ADR 0069); `?sort=` choosing one of the two published orders — `title`, the default, or `newest`, the training's own age (ADR 0071); `?page=`, `?pageSize=`. `200`, `400` when the term is longer than a title, the topic or the sort is unknown, or the page is out of range (ADR 0059) |
...
| `GET` | `/Catalog/topics` | **Anonymous.** The catalog's facets: each topic at least one offered training declares, with its count, alphabetically. Counted over the same composed visibility the search reads, so a facet never promises a shelf the search would answer empty; a topic nothing offered declares is absent rather than zero (ADR 0069). `200` |
...
Twenty-nine endpoints, and not one of them lets a trainer reach what another trainer owns. The eight

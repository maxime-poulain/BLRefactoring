...
| `GET` | `/Catalog/trainings` | **Anonymous.** One page of the offered catalog, by title and shelf. `?term=` matched against the words of a title, each by prefix, through the search index rather than the trainings table; `?topic=` narrowing to one shelf, refused when the domain does not spell it (ADR 0069); `?page=`, `?pageSize=`. `200`, `400` when the term is longer than a title, the topic is unknown or the page is out of range (ADR 0059) |
...
Twenty-seven endpoints, and not one of them lets a trainer reach what another trainer owns. The eight

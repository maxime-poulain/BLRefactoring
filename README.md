...
`Guid.Empty` is rejected at construction, and instances are built through `Create` or `Generate`
— backed by a compiled expression cached per type. Identifiers are generated by the caller before
...
| `TrainerDeletedDomainEvent` | `TrainerId` | A trainer is marked for deletion |
...
| Delete trainer | `TrainerApplicationService.DeleteAsync` | `DeleteTrainerCommand` | Application |
...
| `ConcurrencyConflict` | -3 | | `InvalidTrainer` | 100 |
| `InvalidTitle` | 1 | | `InvalidEmail` | 101 |
| `DuplicateTitle` | 2 | | `BioEmpty` | 102 |
| `InvalidDescription` | 3 | | `BioExceeds500Characters` | 103 |
...
| `GET` | `/Trainer/all` | `200` |
| `DELETE` | `/Trainer/{id}` | `204` or `404`. Also deletes the trainer's trainings, through the domain event |
...
Trainers are created only through registration; there is no `POST /Trainer`.
...
| Project | Scope | Tests |
|---|---|---|
| `BLRefactoring.Shared.Domain.Tests` | Aggregates, value objects, typed identifiers, `Result`, specifications | 231 |
